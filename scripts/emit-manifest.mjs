...
const manifest = {
  ...
  golden: { path: '/projects/git-viewer/', expect: 'Bilko Bibitkov' },
  health: {},
  bundle: { sizeBytesGz: totalGz, fileCount: files.length },
};

writeFileSync(join(distDir, 'manifest.json'), JSON.stringify(manifest, null, 2));
console.log(`emit-manifest: git-viewer ${pkg.version} (${sha}, kit ${hostKitVer}, ${(totalGz / 1024).toFixed(1)} KB gz, ${files.length} files)`);
