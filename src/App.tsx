...
function Shell() {
  ...
  const tabBody = view !== 'home' ? (
    <ProjectDetail id={view} onBack={onHome} variant={variant} setVariant={setVariant} />
  ) : tab === 'overview' ? (
    <OverviewTab
      variant={variant}
      setVariant={setVariant}
      showPipeline={tw.showPipeline}
      onProject={onProject}
    />
  ) : tab === 'projects' ? (
    <ProjectsTab onProject={onProject} />
  ) : tab === 'activity' ? (
    <ActivityTab />
  ) : (
    <WritingTab />
  ...
  return (
    <>
      <Topbar tab={tab} setTab={setTab} onHome={onHome} />
      {tabBody}
